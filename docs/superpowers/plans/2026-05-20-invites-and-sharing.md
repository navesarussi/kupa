...
export function renderFriendInvite(
    ...
    const body = `
        ...
        <a class="btn primary" href="com.kupa.mobile://invite/i/${escapeHtml(token)}">פתח את Kupay</a>
        ${platformButtons()}
...
export function renderGroupInvite(
    ...
    const body = `
        ...
        <a class="btn primary" href="com.kupa.mobile://invite/g/${escapeHtml(token)}">הצטרף לקבוצה ב-Kupay</a>
        ${platformButtons()}
        <p class="footnote">אחרי ההורדה — חזור לקישור הזה.</p>
    `;
    return shell({
