...
**Goal:** Ship link-based invitations for Kupa — both personal friend invite links (`https://kupa.pro/i/<token>`) and group invite links (`https://kupa.pro/g/<token>`) — with auto-join semantics and rotation-based security, surfaced across Settings/EditProfile/Friends/FindFriends/EditGroup/GroupDetail/AddMembersSheet.
...
"invite": {
    "friend": {
        "title": "Invite a friend to Kupa",
        "subtitle": "Send a link via WhatsApp and more",
        "cta": "Share link",
        "shareMessage": "Hey! Let's split expenses together on Kupa. If you sign up through this link we'll automatically become friends: {{url}}",
        ...
        "findInviteName": "Invite {{name}} to Kupa"
    },
    "group": {
        "title": "Share group link",
        "shareMessage": "I added you to the '{{groupName}}' group on Kupa. Join via this link: {{url}}",
...
"invite": {
    "friend": {
        "title": "הזמן חבר ל-Kupa",
        "subtitle": "שלח קישור דרך וואצאפ ועוד",
        "cta": "שתף קישור",
        "shareMessage": "היי! בוא נחלק הוצאות יחד דרך Kupa. אם תרשם דרך הקישור הזה — נהיה אוטומטית חברים: {{url}}",
        ...
        "findInviteName": "הזמן את {{name}} ל-Kupa"
    },
    "group": {
        "title": "שתף קישור לקבוצה",
        "shareMessage": "הוספתי אותך לקבוצת '{{groupName}}' ב-Kupa. הצטרף דרך הקישור: {{url}}",
...
"ios": {
    "supportsTablet": true,
    "bundleIdentifier": "com.kupa.mobile",
    "associatedDomains": ["applinks:kupa.pro"],
    "infoPlist": {
        "CFBundleDisplayName": "Kupa"
    }
}
```
...
export function renderRoot(): string { return '<html><body>Kupa</body></html>'; }
EOF
...
- `KUPA_IOS_TEAM_ID`
- `KUPA_ANDROID_DEBUG_SHA256`
- `KUPA_ANDROID_RELEASE_SHA256`
...
Expected: `applinks.details[0].appID` matches `<TEAM_ID>.com.kupa.mobile` and paths include `/i/*` and `/g/*`.
